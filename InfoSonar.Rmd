...
library('mlbench')
data("Sonar")
b <- sample(c(1:208), size = 9, replace = FALSE, prob = NULL)
print ("kable(head(Sonar[b, 1:10]), format = 'markdown')")
kable(head(Sonar[b, 1:10]), format = "markdown")
print ("kable(head(Sonar[h, 52:61]), format = 'markdown')")
kable(head(Sonar[b, 52:61]), format = "markdown")
```
...
![*Signal by spectral sample and "outcome" (mine or rock)*](https://i.imgur.com/OLzwGGr.gif)
...
![*Signal by return and "outcome" (mine or rock)*](https://i.imgur.com/mJt8gcd.gifv)
...
## library('animation')
for (i in 1:60) {
  filename <- paste ("V", i,".png", sep =  "")
  files <- paste ("Figures/SONAR1", "/", filename, sep = "")
  # print(files)
}
```
